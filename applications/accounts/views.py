    ...
    def post(self, request, *args, **kwargs):
        ...
        if user is not None and not user.is_verified_email:
            ...
            send_custom_email_task.delay(
                user.email, "Подтверждение регистрации", "email_template.html", context
            )

            return Response(
                {"user": user.email, "role": user.role, "status": status.HTTP_200_OK}
            )
...
class ResetPasswordAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        send_custom_email_task.delay(
            user.email, "Сброс пароля", "password_reset_email.html", context
        )
...
class VerifyEmailAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if (
                user.verification_code_created_at + timezone.timedelta(minutes=5)
                < timezone.now()
            ):
                return Response(
                    {
                        "error": "Registration code expired. Please request a new code and try again"
                    },
            ...
            user.save()
            return Response(
                {"status": status.HTTP_200_OK, "user": user.email, "role": user.role}
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user:
            if not user.is_verified_email:
                return Response(
                    {
                        "error": "Please verify your email address by clicking on the confirmation link sent to your inbox"
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
